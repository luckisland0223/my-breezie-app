...
export async function POST(request: NextRequest) {
  try {
    ...
    await prisma.user.update({ where: { id: user.id }, data: { emailVerificationCode: code, emailVerificationExpiresAt: expiresAt } })
    try { await sendVerificationCodeEmail(email, code) } catch (e) { console.error('Resend email failed', e) }
